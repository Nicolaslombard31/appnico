...
export function AuthProvider({ children }: { children: React.ReactNode }) {
        ...
        if (userData) {
          try {
            ...
            if (userInfo && userInfo.id && userInfo.email) {
              ...
              console.error("Erreur: Données utilisateur invalides");
              await signOut();
            }
          } catch (error) {
            console.error("Erreur lors de la conversion des informations de l'utilisateur", error);
            await signOut();
          }
        }
      } catch (error) {
  ...
  const signIn = async (token: string, userData: User) => {
    try {
      ...
      const secureStroreOptions = {
        keychainAccessible: SecureStore.AFTER_FIRST_UNLOCK,
      };

      await SecureStore.setItemAsync(SECURE_TOKEN_KEY, token, secureStroreOptions);
      await SecureStore.setItemAsync(SECURE_USER_DATA_KEY, JSON.stringify(userData), secureStroreOptions);
